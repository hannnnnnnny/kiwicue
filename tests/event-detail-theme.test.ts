...
describe("event detail visual contract", () => {
  it("uses a readable editorial desktop layout with a sticky venue panel", () => {
    expect(css).toMatch(/\.event-detail-shell\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1\.35fr\)\s+minmax\(320px,\s*\.65fr\)/s);
    expect(css).toMatch(/\.event-detail-venue\s*\{[^}]*position:\s*sticky[^}]*top:\s*20px/s);
  ...
  it("becomes one column at 820px and disables sticky positioning on short viewports", () => {
    expect(css).toMatch(/@media \(max-width:\s*820px\)[\s\S]*\.event-detail-shell\s*\{[^}]*grid-template-columns:\s*1fr/s);
