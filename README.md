...
The URL and publishable key are intended for browser clients. The secret key is used only on the server for account deletion and the short-lived signup confirmation check; it must never have a `NEXT_PUBLIC_` prefix. The legacy `SUPABASE_SERVICE_ROLE_KEY` remains supported as a fallback, but new installations should use `SUPABASE_SECRET_KEY`. Configure Supabase Auth Site URL and exact redirect allowlist entries for `/auth/callback?next=/reset-password` and `/auth/confirmed` on each enabled origin. Email/password sign-up, recovery and account deletion require working Auth email delivery and correctly applied migrations. Do not enable account UI in production before RLS policies and deletion have been checked with two separate test users.

After signup, the original browser tab checks confirmation for at most 15 minutes. Confirming the email on a phone opens a separate confirmation page; the original tab signs in automatically once verified. The password stays only in that open tab's memory. If the tab is closed or times out, use normal login; no password or auth session is transferred between devices.
